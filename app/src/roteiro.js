...
var Roteiro = (function () {
  ...
  function html() {
    return '<section class="panel rt-painel">' +
        ...
        '<div class="rt-tabs" role="tablist">' +
          ...
          '<button class="rt-tab" role="tab" aria-selected="false" data-rtaba="abordagem">Gerar abordagem</button>' +
        ...
        '<div id="rtObj" hidden>' +
          ...
          '</div>' +
        '</div>' +
        '<div id="rtAbordagem" hidden>' +
          '<p class="rt-dica">Descreva o lead — mesmo sem cliente escolhido na lista — e ' +
            'receba abertura, primeira pergunta, respostas prováveis e a próxima ação.</p>' +
          '<div class="rt-ab-form">' +
            '<label class="rt-ab-rot" for="rtAbNome">Cliente / empresa</label>' +
            '<input class="rt-ab-campo" id="rtAbNome" maxlength="120" placeholder="Nome do integrador ou da empresa">' +
            '<div class="rt-ab-linha2">' +
              '<div><label class="rt-ab-rot" for="rtAbCidade">Cidade</label>' +
                '<input class="rt-ab-campo" id="rtAbCidade" maxlength="120"></div>' +
              '<div><label class="rt-ab-rot" for="rtAbEstado">Estado</label>' +
                '<input class="rt-ab-campo" id="rtAbEstado" maxlength="40" placeholder="UF"></div>' +
            '</div>' +
            '<div class="rt-ab-linha2">' +
              '<div><label class="rt-ab-rot" for="rtAbCanal">Canal</label>' +
                '<select class="rt-ab-campo" id="rtAbCanal">' +
                  '<option value="">— não informado —</option>' +
                  '<option value="Telefone">Telefone</option>' +
                  '<option value="WhatsApp">WhatsApp</option>' +
                '</select></div>' +
              '<div><label class="rt-ab-rot" for="rtAbComprou">Já comprou da BelEnergy?</label>' +
                '<select class="rt-ab-campo" id="rtAbComprou">' +
                  '<option value="">Não sei</option>' +
                  '<option value="sim">Sim</option>' +
                  '<option value="nao">Nunca</option>' +
                '</select></div>' +
            '</div>' +
            '<label class="rt-ab-rot" for="rtAbUltima">Última compra / última interação</label>' +
            '<input class="rt-ab-campo" id="rtAbUltima" maxlength="120" placeholder="Ex.: 20/08/2026, ou &quot;nunca conversamos&quot;">' +
            '<label class="rt-ab-rot" for="rtAbObs">O que você já sabe</label>' +
            '<textarea class="rt-ia-txt" id="rtAbObs" maxlength="800" rows="3" ' +
              'placeholder="Ex.: apareceu numa busca de integradores da região, atende obra residencial…"></textarea>' +
          '</div>' +
          '<div class="btn-row" style="margin-top:10px">' +
            '<button class="btn btn-sm rt-btn-ia" id="rtAbGerar">Gerar abordagem</button>' +
            '<span class="rt-ia-conta" id="rtAbConta"></span>' +
          '</div>' +
          '<div id="rtAbSaida"></div>' +
        '</div>' +
  ...
  function trocarAba(qual) {
    ...
    document.getElementById('rtAbordagem').hidden = qual !== 'abordagem';
  ...
  /* ---------- gerar abordagem ----------
     Diferente das abas "Como abrir"/"Se ele disser que...", que sao roteiro
     fixo, esta gera um plano sob medida a partir do que o vendedor escreveu
     -- por isso tem orcamento e trava proprios (mesmo padrao de "pergunta a
     IA", rota /api/abordagem em vez de /api/duvida). */

  var ABORDAGEM_ROTULO = {
    fria: 'Prospecção fria', morna: 'Prospecção morna', nunca_comprou: 'Nunca comprou',
    inativo: 'Inativo', reativacao: 'Reativação', ativo_crescimento: 'Ativo — crescimento',
    cross_sell: 'Cross-sell', upsell: 'Upsell', recuperacao: 'Recuperação'
  };
  var TEMPERATURA_ROTULO = { quente: 'Quente', morno: 'Morno', frio: 'Frio', nao_qualificado: 'Não qualificado' };

  /* O cliente escolhido na lista preenche o que ja se sabe dele; o resto
     fica em branco para o vendedor completar. Redesenha do zero a cada troca
     de cliente, igual as outras abas -- e uma escolha deliberada, nao um
     lead qualquer que se guarda entre cliques. */
  function preencherFormAbordagem() {
    var nome = document.getElementById('rtAbNome');
    if (!nome) return;
    nome.value = atual ? (atual.nomeCompleto || atual.rotulo || '') : '';
    document.getElementById('rtAbCidade').value = (atual && atual['{CIDADE}']) || '';
    document.getElementById('rtAbEstado').value = '';
    document.getElementById('rtAbCanal').value = '';
    document.getElementById('rtAbComprou').value = '';
    var pista = [];
    if (atual && atual['{ULTIMA}']) pista.push(atual['{ULTIMA}']);
    if (atual && atual['{DIAS}']) pista.push('parado há ' + atual['{DIAS}']);
    document.getElementById('rtAbUltima').value = pista.join(' — ');
    document.getElementById('rtAbObs').value = '';
    document.getElementById('rtAbSaida').innerHTML = '';
  }

  function desenharSaldoAbordagem(dados) {
    var conta = document.getElementById('rtAbConta');
    var botao = document.getElementById('rtAbGerar');
    if (!conta || !botao) return;
    if (!dados || !dados.ligado) { botao.hidden = true; conta.textContent = ''; return; }
    botao.hidden = false;
    var restam = Number(dados.restantes) || 0;
    conta.textContent = restam + ' de ' + dados.limite + ' restantes hoje';
    conta.setAttribute('data-baixo', restam <= 2 ? '1' : '0');
    botao.disabled = restam <= 0;
  }

  function barraQualif(rotulo, valor) {
    var v = Math.max(0, Math.min(100, Number(valor) || 0));
    return '<div class="rt-qbarra"><span class="rt-qbarra-rot">' + rtEsc(rotulo) + '</span>' +
      '<div class="rt-qbarra-trilha"><div class="rt-qbarra-fill" style="width:' + v + '%"></div></div>' +
      '<span class="rt-qbarra-n">' + v + '</span></div>';
  }

  function desenharAbordagemResultado(r, restantes) {
    var saida = document.getElementById('rtAbSaida');
    saida.innerHTML = '';
    var cx = el('div', 'rt-ia-resposta');

    cx.appendChild(el('span', 'rt-ab-tipo', rtEsc(ABORDAGEM_ROTULO[r.tipoAbordagem] || r.tipoAbordagem || '—')));

    if (r.perfilProvavel) {
      cx.appendChild(el('div', 'rt-etapa', 'Perfil provável'));
      cx.appendChild(el('p', 'rt-portras', rtEsc(r.perfilProvavel)));
    }
    if (r.hipoteseComercial) {
      cx.appendChild(el('div', 'rt-etapa', 'Hipótese comercial'));
      cx.appendChild(el('p', 'rt-portras', rtEsc(r.hipoteseComercial)));
    }

    cx.appendChild(el('div', 'rt-etapa', 'Abertura'));
    cx.appendChild(el('div', 'rt-fala', rtEsc(r.abertura)));
    var linhaAb = el('div', 'btn-row');
    var copiarAb = el('button', 'btn btn-sm btn-ghost', 'Copiar abertura');
    copiarAb.addEventListener('click', function () { copiar(r.abertura, copiarAb); });
    linhaAb.appendChild(copiarAb);
    cx.appendChild(linhaAb);

    if (r.pergunta) {
      cx.appendChild(el('div', 'rt-etapa', 'Primeira pergunta'));
      cx.appendChild(el('div', 'rt-fala', rtEsc(r.pergunta)));
    }

    (r.possiveisRespostas || []).forEach(function (par) {
      var caixa = el('div', 'rt-ab-par');
      caixa.appendChild(el('div', 'rt-ab-par-cliente', '“' + rtEsc(par.respostaCliente) + '”'));
      caixa.appendChild(el('div', 'rt-fala', rtEsc(par.respostaVendedor)));
      cx.appendChild(caixa);
    });

    if (r.proximoPasso) {
      cx.appendChild(el('div', 'rt-etapa', 'Próximo passo'));
      cx.appendChild(el('p', 'rt-porque', rtEsc(r.proximoPasso)));
    }

    var q = r.qualificacao || {};
    cx.appendChild(el('div', 'rt-etapa', 'Qualificação — ' + rtEsc(TEMPERATURA_ROTULO[q.temperatura] || q.temperatura || '—')));
    cx.appendChild(el('div', null,
      barraQualif('Potencial', q.potencial) + barraQualif('Intenção', q.intencao) + barraQualif('Urgência', q.urgencia)));

    var a = r.proximaAcao || {};
    if (a.acao) {
      cx.appendChild(el('div', 'rt-etapa', 'Próxima ação'));
      cx.appendChild(el('p', 'rt-portras', '<b>' + rtEsc(a.acao) + '</b>' + (a.prazo ? ' — ' + rtEsc(a.prazo) : '')));
      if (a.objetivo) cx.appendChild(el('p', 'rt-porque', rtEsc(a.objetivo)));
      if (a.mensagem) {
        cx.appendChild(el('div', 'rt-fala', rtEsc(a.mensagem)));
        var linhaAcao = el('div', 'btn-row');
        var copiarAcao = el('button', 'btn btn-sm btn-ghost', 'Copiar mensagem');
        copiarAcao.addEventListener('click', function () { copiar(a.mensagem, copiarAcao); });
        linhaAcao.appendChild(copiarAcao);
        cx.appendChild(linhaAcao);
      }
    }

    cx.appendChild(el('p', 'rt-rodape-ia',
      'Plano gerado agora, só com o que foi preenchido acima. <strong>Não sabe preço, prazo, ' +
      'estoque nem condição do mês</strong> — confirme com o gestor antes de prometer isso ao cliente.'));

    if (restantes === 0) {
      cx.appendChild(el('div', 'rt-cuidado', '<b>Acabaram suas abordagens geradas hoje.</b> Volta amanhã.'));
    }
    saida.appendChild(cx);
  }

  function gerarAbordagem() {
    var botao = document.getElementById('rtAbGerar');
    var saida = document.getElementById('rtAbSaida');
    var lead = {
      nome: document.getElementById('rtAbNome').value.trim(),
      cidade: document.getElementById('rtAbCidade').value.trim(),
      estado: document.getElementById('rtAbEstado').value.trim(),
      canal: document.getElementById('rtAbCanal').value,
      ultimaCompra: document.getElementById('rtAbUltima').value.trim(),
      observacoes: document.getElementById('rtAbObs').value.trim()
    };
    var comprou = document.getElementById('rtAbComprou').value;
    if (comprou === 'sim') lead.jaComprou = true;
    else if (comprou === 'nao') lead.jaComprou = false;

    if (!lead.nome && !lead.cidade && !lead.observacoes) {
      saida.innerHTML = '';
      saida.appendChild(el('div', 'rt-cuidado', 'Preencha ao menos o nome, a cidade ou "o que você já sabe".'));
      return;
    }
    ...
    saida.appendChild(el('div', 'rt-pensando',
      '<span class="rt-ponto"></span><span class="rt-ponto"></span>' +
      '<span class="rt-ponto"></span><span>Montando a abordagem…</span>'));

    ponte.gerarAbordagem('gerar', lead).then(function (dados) {
      botao.disabled = false;
      desenharAbordagemResultado(dados.resposta, dados.restantes);
      var conta = document.getElementById('rtAbConta');
      if (conta && typeof dados.restantes === 'number') {
        conta.textContent = dados.restantes + ' restantes hoje';
        conta.setAttribute('data-baixo', dados.restantes <= 2 ? '1' : '0');
        botao.disabled = dados.restantes <= 0;
      }
    }, function (e) {
      botao.disabled = false;
      saida.innerHTML = '';
      saida.appendChild(el('div', 'rt-cuidado', rtEsc((e && e.message) || 'Não consegui gerar agora.')));
    });
  }
  ...
  return {
    html: html,
    daLinha: daLinha,
    /* Abrir o caderno tem que ser VISIVEL. No celular o roteiro fica acima
       das listas, entao o vendedor que rolou ate o cliente 20 e deu o toque
       duplo abria a aba a mil pixels de distancia -- a tela nao mexia e
       parecia que o duplo clique nao funcionava. Aqui a pagina vai junto, e
       o cursor cai direto na linha de escrever. */
    abrirNotas: function () {
      trocarAba('notas');
      var painel = document.getElementById('rtNotas');
      if (!painel) return;
      var caixa = document.querySelector('.rt-painel') || painel;
      var r = caixa.getBoundingClientRect();
      var jaNaTela = r.top >= 0 && r.top < (window.innerHeight || 0) * 0.6;
      if (!jaNaTela) {
        try { caixa.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
        catch (e) { caixa.scrollIntoView(); }
      }
      var campo = painel.querySelector('tr[data-rtnova="1"] .rt-nota-txt');
      if (campo) {
        setTimeout(function () {
    ...
    iniciar: function (nomeVendedor, funcoes) {
      ...
      var gerarBtn = document.getElementById('rtAbGerar');
      if (gerarBtn) gerarBtn.addEventListener('click', gerarAbordagem);
      preencherFormAbordagem();
    ...
    saldoAbordagem: function () {
      if (!ponte.gerarAbordagem) return;
      ponte.gerarAbordagem('saldo').then(desenharSaldoAbordagem, function () {
        var botao = document.getElementById('rtAbGerar');
