...
const SUPERFLUAS = new Set(VERBOS_SAIDA.concat(VERBOS_ENTRADA).concat([
  'reais', 'real', 'conto', 'contos', 'pila', 'centavos', 'centavo',
  ...
  function ligar() {
    ...
    rec.onresult = (ev) => {
      ...
      for (let i = ev.resultIndex; i < ev.results.length; i++) {
        ...
        if (r.isFinal) {
          if (o.onTexto) o.onTexto(texto.trim(), { confianca: r[0] && r[0].confidence });
        } else {
          parcial += texto;
        }
      }
      if (parcial && o.onParcial) o.onParcial(parcial.trim());
    };
